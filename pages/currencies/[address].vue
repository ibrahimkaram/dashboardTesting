            ...
            <div class="">
              <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-2" aria-label="Tabs">
                  <a v-for="tab in tabs" :key="tab.name" :class="[tabId===tab.id ? 'border-primary-blue text-primary-blue' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300', 'group inline-flex items-center py-4 px-1 border-b-2 font-medium text-sm']" :aria-current="tabId===tab.id ? 'page' : undefined">
                    <button @click="switchTabs(tab.id)" class="flex" >
                      <component :is="tab.icon" :class="[tabId===tab.id ? 'text-primary-blue' : 'text-gray-400 group-hover:text-gray-500', '-ml-0.5 mr-2 h-8 w-8']" aria-hidden="true" />
